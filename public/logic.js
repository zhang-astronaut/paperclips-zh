...
function hasPresige() {
  return localStorage.getItem('savePrestige') != null;
}
...
function save( slotNumber = 0 ) {
  var slot = slotNumber ? slotNumber.toString() : '';
  ...
  localStorage.setItem(`saveProjectsUses${slot}`,JSON.stringify(projectsUses));
  localStorage.setItem(`saveProjectsFlags${slot}`,JSON.stringify(projectsFlags));
  localStorage.setItem(`saveProjectsActive${slot}`,JSON.stringify(projectsActive));
  localStorage.setItem(`saveStratsActive${slot}`,JSON.stringify(stratsActive));
}
...
function load( slotNumber = 0 ) {
  var slot = slotNumber ? slotNumber.toString() : '';

  var loadGame = JSON.parse(localStorage.getItem(`saveGame${slot}`));
  var loadProjectsUses = JSON.parse(localStorage.getItem(`saveProjectsUses${slot}`));
  var loadProjectsFlags = JSON.parse(localStorage.getItem(`saveProjectsFlags${slot}`));
  var loadProjectsActive = JSON.parse(localStorage.getItem(`saveProjectsActive${slot}`));
  var loadStratsActive = JSON.parse(localStorage.getItem(`saveStratsActive${slot}`));
...
function incrementClipRateTracker() {
  clipRateTracker++;
}

function updateClipRate() {
  if (clipRateTracker < 100){
    clipRateTemp = clipRateTemp + (clips - prevClips);
    prevClips = clips;
  } else {
    clipRateTracker = 0;
...
function synchroniseStratPicker(){
